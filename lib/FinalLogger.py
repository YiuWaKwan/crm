...
def getLogger(fileName,maxByte=700*1024*1024,timeType='d',backupCount=10,console=True,fileLogLevel=-1,conLogLevel=-1,type='time'):
    FILE_LOGGING_MSG_FORMAT = logging.Formatter("%(asctime)s %(levelname)s %(filename)s %(funcName)s (line:%(lineno)d)[%(process)d][%(threadName)s][%(message)s]")
    CONSOLE_LOGGING_MSG_FORMAT = logging.Formatter("%(asctime)s %(levelname)s %(filename)s %(funcName)s (line:%(lineno)d)[%(process)d][%(threadName)s][%(message)s]")
