var appWx = angular.module("appWx", ["ui.router","ngSanitize","ngAnimate","ngWebSocket"]).factory('NoticeData', function($websocket) {
...
appWx.config(function($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider.state('contentContact', {
        ...
        views:{
            'contentView':{
                templateUrl: "/static/template/contentChat.html"
            }
        }
    }).state('searchList', {
        url:'/searchList',
...
appWx.controller('appController', function($scope, $state, $interval,$http,NoticeData,$timeout, $templateCache) {
    ...
    $scope.flushDate=function() {
        try {
            ...
            if ($scope.account.wx_id in $scope.NoticeData.getwx()) {
                ...
                angular.forEach($scope.chatList, function (chat, key) {
                    ...
                    if (chat.wx_id in wxNotice) {
                        ...
                        delete wxNotice[chat.wx_id];
                        if ($scope.currentContact.wx_id == chat.wx_id) {
            ...
            $scope.groupChatAtCount = 0;
            angular.forEach($scope.groupChatNoticeAtCtrls.groupChatNoticeAt, function (groupChatNoticeAtCtrl, key) {
                var atId = key;
                var wxMainId = atId.split("#")[0];
                var chatGroupId = atId.split("#")[1];
                var wxId = atId.split("#")[2];
                if (!(atId in $scope.wxInfosAt)) {
                    angular.forEach($scope.wxInfos, function (wxInfo, key) {//获取不存在列表中的微信主号信息
                        if (wxInfo["wx_id"] == wxMainId) {
                            $scope.wxInfosAt[atId] = {
                                wx_id: wxInfo.wx_id,
                                NickName: wxInfo.NickName,
                                NoticeCount: $scope.groupChatNoticeAtCtrls.groupChatNoticeAt[atId],
                                RemarkName: wxInfo.RemarkName,
                                Sex: wxInfo.Sex,
                                chatGroupAtWxName: wxInfo.chatGroupAtWxName,
                                chatGroupId: wxInfo.chatGroupId,
                                chatGroupName: wxInfo.chatGroupName,
                                groupName: wxInfo.groupName,
                                if_start: wxInfo.if_start,
                                phone_no: wxInfo.phone_no,
                                pic: wxInfo.pic,
                                pic_big: wxInfo.pic_big,
                                signature: wxInfo.signature,
                                wx_id: wxInfo.wx_id,
                                zone: wxInfo.zone,
                                chatGroupId: chatGroupId,
                                atId: atId,
                                msgId:$scope.groupChatNoticeAtCtrls.chatGroupAtMsgIds[atId],//只显示最旧的未读msgId
                                chatGroupName: $scope.groupChatNoticeAtCtrls.chatGroupNames[atId],
                                chatGroupAtWxName: $scope.groupChatNoticeAtCtrls.chatGroupAtWxNames[atId]
                            };
                        }
                    });
                }
                var currentWxMainNoticeCount = $scope.groupChatNoticeAtCtrls.groupChatNoticeAt[key];
                $scope.wxInfosAt[atId]["NoticeCount"] = currentWxMainNoticeCount;
                $scope.groupChatAtCount += currentWxMainNoticeCount;
            });
            //清除已读过的@的消息数量提示
            angular.forEach($scope.wxInfosAt, function (wxInfoAt, key) {
                if (!$scope.groupChatNoticeAtCtrls.groupChatNoticeAt[key]) {
                    wxInfoAt.NoticeCount = 0;
                }
            });
        ...
        //$scope.timer=$timeout(function(){ $scope.flushDate();}, 2000);
    }
   ...
   $scope.getAnnounceData=function(){//回调函数中只能传index,传aSeqId则会传全局变量覆盖每个关闭层的aSeqId,都是一样的值导致写浏览记录不正常
    ...
    $scope.chooseWxAt=function(wxKey,wxValue,msgId){
        var wxMainId=wxKey.split("#")[0];
        var wxGroupId=wxKey.split("#")[1];
        var wxId=wxKey.split("#")[2];
        //var msgId=wxKey.split("#")[3];
        var chooseWxParam={};
        chooseWxParam["flag"]=1;
        chooseWxParam["wxId"]=wxGroupId;
        chooseWxParam["msgId"]=msgId;

        $scope.isClickAt=msgId;
        $scope.chooseWx(wxValue,chooseWxParam);
        //清理数据  改到消息显示逻辑
        //$scope.clearAtMsg(wxMainId,wxGroupId,wxId);
    }
    ...
    $scope.chooseWx=function(wx,chooseWxParam){
        ...
        $http({
            ...
            data:{}
                ...
                if(chooseWxParam&&chooseWxParam.flag==1){//有@消息，自动点击时触发
                 var isGoOn=true;
                 angular.forEach($scope.chatList, function(chat, key) {
                    if(isGoOn){
                        if(chat.wx_id == chooseWxParam.wxId){
                            $scope.chatShow(chat);
                            isGoOn=false;
                        }
                    }
    ...
    //聊天和联系人切换
    $scope.clickChat=function(current_name){$scope.state_current_name=current_name;}
    //联系人详细信息显示
    ...
    $scope.chatShowing=function (messageList) {
        ...
        if (contactItem.NoticeCount && contactItem.NoticeCount > 0) {
            //清理主号和分组的通知数量
            angular.forEach($scope.wxInfos, function(item, key) {
                if ($scope.account.wx_id == item.wx_id) {
                    item.NoticeCount=$scope.account.NoticeCount-contactItem.NoticeCount;
                    if(item.NoticeCount < 0){item.NoticeCount=0;}
                    angular.forEach($scope.groups, function(group, key) {
                        if(group.groupName == item.groupName){
                            group.NoticeCount=group.NoticeCount-contactItem.NoticeCount;
                            if(group.NoticeCount < 0){group.NoticeCount=0;}
                        }
                    });
                }
            });
            ...
            angular.forEach($scope.chatList, function(item, key) {
                if (contactItem.wx_id == item.wx_id) {
                    item.NoticeCount=0;
                }
            });
            ...
            $.ajax({
                ...
                url: "http://"+window.location.host+"/WXCRM/wxChatInfo/",
                async: false,
                data: {"oper":"loadChat","wx_id":$scope.currentContact.wx_id,"wx_main_id":$scope.account.wx_id},
                success: function (result) {
                    severcheck(result);
                    result = parseJson(result);
        ...
        }else{//如果当前微信号无通知消息，则把@中没被清理的通知给清理了
             angular.forEach($scope.groupChatNoticeAtCtrls.groupChatNoticeAt, function (groupChatNoticeAtCtrl, key) {
                var atId = key;
                var wxMainId = atId.split("#")[0];
                var chatGroupId = atId.split("#")[1];
                var wxId = atId.split("#")[2];
                if (wxMainId==$scope.account.wx_id&&chatGroupId==$scope.currentContact.wx_id) {
                    $scope.clearAtMsg($scope.account.wx_id, chatGroupId, wxId);
                }
            });
        }
    ...
    $scope.chatNoticeShowing=function () {
        ...
        if (contactItem.NoticeCount && contactItem.NoticeCount > 0) {
            ...
            $.ajax({
                ...
                url: "http://"+window.location.host+"/WXCRM/wxChatInfo/",
                async: false,
                data: {"oper":"loadChat","wx_id":$scope.currentContact.wx_id,"wx_main_id":$scope.account.wx_id,"isCurrent":1}, //#isCurrent:如果是当前聊天界面加载消息，为避免消息遗漏，删除时根据msgID删
                success: function (result) {
                    severcheck(result);
                    result = parseJson(result);
    ...
    $scope.sendTextMessage=function(fileObject){
        if($("#editArea").html().trim() == '' && !fileObject && $scope.noticeList.length==0){
            return false;
        }
        var myDate = new Date();
        ...
        if(!update){
            ...
            chatItem={'show':true,'MMIsSend':true,'NickName':$scope.currentContact.NickName,'wx_id':$scope.currentContact.wx_id, 'pic':$scope.currentContact.pic, 'pic_big':$scope.currentContact.pic, 'isTop':$scope.currentContact.isTop, 'SendMin':sendMinFull, 'NoticeCount':0, 'LastChart':message.LastChart,RemarkName:$scope.currentContact.RemarkName}
        ...
        $.ajax({
            ...
            success: function (result) {severcheck(result);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown, data) {
    ...
    $scope.modifyWx=function(modify_type, wx_id){
        ...
        if(modify_type == "flush_chat"){
            ...
            if(!flag){$scope.noticeList.push({"name":$scope.contextMenuFlag.headname})};
            $scope.memberListUpdate();
            return false;
        }
...
*/
    ...
    $scope.addNoticeName=function(headname){
        var flag=false;
        headname="@"+headname+" ";
        angular.forEach($scope.noticeList, function(notice, key1) {
            if(notice.name == headname){
                flag=true;
            }
        });
        if(!flag){$scope.noticeList.push({"name":headname})};
        $scope.memberListUpdate();
    }
