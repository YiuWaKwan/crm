window.onload = function () {
    ...
    var start_time= new Date(new Date().setDate(new Date().getDate()-1)).format('yyyyMMdd');
    var end_time=new Date().format('yyyyMMdd');
        //执行一个laydate实例
    laydate.render({
    ...
    getWxId();
    data_sum(start_time,end_time);
    group_num_change(start_time,end_time);
    active_change(start_time,end_time);
    get_group_num();
...
function data_sum(start_time,end_time) {
    $.ajax({
        type: "POST",
        url: "../get_sdata_sum/",
        ...
        success: function (data) {
                ...
                $('#not_active_num').html(data[0]['not_active_num']);
                $('#out_group_num').html(data[0]['out_group_num']);
                $('#in_group_num').html(data[0]['in_group_num']);
...
function group_num_change(start_time,end_time) {
        $.ajax({
            type: "POST",
            url: "../group_num_change/",
            data: {'group_list': selected_id_list_str,'wx_id':wx_id,'start': start_time, 'end': end_time},
            success: function (data) {
                severcheck(data);
                data = parseJson(data);
                ...
                for (x in data) {
                    ...
                    ydata2.push(data[x]['in_group_num']);
                    ydata3.push(data[x]['out_group_num']);
                    }
                var myChart = echarts.init(document.getElementById('group_num_change'));
                var option = {
                    toolbox: {
                    feature: {
                        dataView: {},
...
function active_change(start_time,end_time) {
        $.ajax({
            type: "POST",
            url: "../active_change/",
            data: {'group_list': selected_id_list_str,'wx_id':wx_id,'start': start_time, 'end': end_time},
            success: function (data) {
                ...
                for (x in data) {
                    ...
                    xdata.push(data[x]['cur_date']);
                    ydata1.push(data[x]['active_num']);
                    ydata2.push(data[x]['active_percent']);
                    }
                var myChart = echarts.init(document.getElementById('active_change'));
                var option = {
                    toolbox: {
                    feature: {
                        dataView: {},
 ...
 // function group_spread(start_time,end_time) {
 //    var group_list = '5459821902@chatroom';
 //    $.ajax({
 //        type: "POST",
 //        url: "../get_group_spread/",
 //        data: {'group_list':group_list,'wx_id':wx_id,'start':start_time,'end':end_time},
 //        success: function (data) {
 //                severcheck(data);
 //                data = parseJson(data);
 //                scaleData = [
